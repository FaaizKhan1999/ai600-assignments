...
\begin{titlepage}
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \honorPledge
    \noindent All work, code implementation, and exploratory data analysis for this assignment can be found at the following public GitHub repository: \\
\url{https://github.com/FaaizKhan1999/ai600-assignments/tree/main/assignment-01}
    \vspace{1.2cm}
  \end{minipage}
\end{titlepage}
...
\begin{question}[Bias Gradients, Parameter Sharing, and Activation Effects]
  ...
  $$\frac{\partial L}{\partial a^{(1)}}=(U^\top\vec{\delta}^{(2)})\odot g(a^{(1)})=\vec{\delta}^{(1)}$$
  $$\frac{\partial a^{(1)}}{\partial b}=I$$
  Putting the two together:
  $$\frac{\partial L}{\partial b}=\vec{\delta}^{(2)}+\vec{\delta}^{(1)}$$
  Or, in terms of $\vec{\delta}^{(2)}$:
  $$\frac{\partial L}{\partial b}=\vec{\delta}^{(2)}+(U^\top\vec{\delta}^{(2)})\odot g(a^{(1)})$$
  Bias sharing is expected to affect both convergence speed and stability:
  \begin{itemize}
    \item Because the gradient is the sum of contributions from multiple paths in bias sharing, the magnitude of updates will be larger, allowing for faster initial learning but making room for instability.
    \item Bias sharing also couples the transformation of the hidden layer with that of the output layer. This effectively reduces the degrees of freedom and constrains the optimization. Since now the model has to find a single bias that satisfies both layers, the model will likely converge slower to a local minimum.
    \item The dependency of the output on the same parameter at different depths can affect the conditioning of the Hessian matrix, potentially making the optimisation more difficult.
  \end{itemize}
\end{question}
